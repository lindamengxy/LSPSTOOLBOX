...
% Last Modified by GUIDE v2.5 22-Aug-2014 16:27:28
...
if ispushed
     ...
     elseif ~isfield(handles,'direct_tValue')||isempty(handles.direct_tValue)
        ...
        handles= mapave_gui_forDan(handles.avgfolder,handles)
        if flag
            display('finished')
            set(hObject, 'String', 'Ready.');
        end
...
handles= MeanValueCalculationSiRatio_HI( handles );
...
%     path_save=handles.save_path;
    ...
    if LaserStim
        ...
        [Stim_coor,header]=Matrixrotate_singlemap(header,flipimg);
        X=Stim_coor(1,:);
        Y=Stim_coor(2,:);
